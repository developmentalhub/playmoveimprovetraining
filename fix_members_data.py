f = open('src/lib/membersData.ts', 'w', encoding='utf-8')
f.write("""import { createServerClient } from '@supabase/ssr';
import { cookies } from 'next/headers';

interface Post { id: string; user_id: string; content: string; parent_id: string | null; created_at: string; replies: Post[]; }
...
export async function getForumPosts(): Promise<Post[]> {
  ...
  if (!data) return [];
  const withReplies: Post[] = await Promise.all(data.map(async (post: Post) => {
    ...
    return { ...post, replies: (replies ?? []) as Post[] };
  }));
  return withReplies;
}
