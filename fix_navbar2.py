    ...
    <div style={{backgroundColor: '#4ABFB0'}} className="shadow-md relative">
      <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-2">
          ...
          <a href="https://developmental-hub.vercel.app" target="_blank" rel="noopener noreferrer" className="text-white font-semibold px-4 py-2 rounded-full text-sm hover:bg-white hover:bg-opacity-20 transition-all">Developmental Hub</a>
          {!loading && user && isMember ? (
            ...
            <>
              <Link href="/videos" className="font-semibold px-4 py-2 rounded-full text-sm transition-all" style={{background: 'rgba(255,255,255,0.2)', color: '#fff'}}>Join $39/month</Link>
              <button onClick={handleSignOut} className="bg-white font-semibold px-4 py-2 rounded-full text-sm shadow" style={{color: '#7B4FA6'}}>Sign Out</button>
            </>
          ) : (
            <>
              ...
              <Link href="/videos" className="font-semibold px-4 py-2 rounded-full text-sm shadow transition-all hover:brightness-110" style={{backgroundColor: '#7B4FA6', color: '#fff'}}>Join $39/month</Link>
      ...
      {menuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 shadow-lg z-50 px-6 py-4 flex flex-col gap-2" style={{backgroundColor: '#3da898'}}>
          ...
          <a href="https://developmental-hub.vercel.app" target="_blank" rel="noopener noreferrer" onClick={() => setMenuOpen(false)} className="text-white font-semibold py-3 border-b border-white border-opacity-20">Developmental Hub</a>
          {!loading && user && isMember ? (
            <>
            ...
            <>
              <Link href="/videos" onClick={() => setMenuOpen(false)} className="text-white font-semibold py-3 border-b border-white border-opacity-20">Join $39/month</Link>
              <button onClick={handleSignOut} className="text-left text-white font-semibold py-3">Sign Out</button>
            </>
          ) : (
            <>
