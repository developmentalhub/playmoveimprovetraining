  ...
  async function handleSubmit() {
    ...
    if (isSignUp) {
      const { error } = await supabase.auth.signUp({
        email,
        password,
        options: { emailRedirectTo: 'https://www.playmoveimprove.com/auth/callback' },
      })
      setLoading(false)
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-16">
        <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md border-t-4" style={{borderColor: '#4ABFB0'}}>
          ...
          <div className="mb-6">
            ...
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()} className="w-full border rounded-lg px-4 py-2 text-sm focus:outline-none" style={{borderColor: '#4ABFB0'}} placeholder="••••••••" />
          </div>
          ...
          {!isSignUp && (
            ...
            }} className="w-full text-sm text-center" style={{color: '#4a5568'}}>
